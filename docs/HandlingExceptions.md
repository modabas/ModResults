...
A failed Result instance can be created from an exception by calling [corresponding static method](../src/ModResults/ResultFailureExtensions.cs) with an exception input parameter for each FailureType, or can be left to implicit operator which creates a Failed Result with FailureType CriticalError by default.
